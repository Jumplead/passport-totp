...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var self = this;
  this._setup(req.user, function(err, key, period, errorMsg) {
    if (err) { return self.error(err); }

    var rv = speakeasy.totp.verify({secret: key, encoding: 'base32', token: value});
    if (!rv) { return self.fail(errorMsg); }
